...
int main(int argc, char **argv) {
    ...
    if (argc != 3) {
    fprintf(stderr, "usage: %s <port> <node_num>\n", argv[0]);
    exit(1);
    }
    portno = atoi(argv[1]);
    ...
    */
    ...
    for (int i=0; i<connectcnt; i++) {
        ...
        error("ERROR on accept");
          connectcnt++;
          server_fds[curr_server++] = childfd;
        ...
        //pthread_t thread;
        //pthread_create(&thread, NULL, ThreadEntry, (void*)(long)server_fds[0]);
